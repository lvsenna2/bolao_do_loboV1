...
export function UserMobileMenu({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  return (
    <>
      ...
      <Modal label="Menu principal" onClose={() => setOpen(false)} open={open}>
        <aside
          onClick={(event) => {
            if ((event.target as Element).closest("a")) setOpen(false);
          }}
          className="relative flex h-full w-full max-w-xs flex-col border-r border-brand-gold/20 bg-[#0b0b0b] text-white shadow-2xl"
        >
          ...
          <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-3 py-4">
            <NavigationList
              className="[&_a]:h-11 [&_a:not([aria-current])]:text-white/75"
          ...
          <div className="border-t border-white/10 p-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
            <div className="flex items-center justify-between rounded-control bg-white/5 px-3 py-2">
              <span className="text-sm text-white/65">Aparência</span>
              <ThemeToggle />
            </div>
          </div>
        </aside>
